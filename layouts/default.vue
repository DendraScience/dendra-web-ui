<template>
  <div :class="isMenuOpen ? 'drawer-left-open' : ''">
    ...
    <div class="bg-faded drawer-left drawer-left-shadow-inset h-100">
      ...
      <section class="px-2 py-2 border-bottom-darken-10" v-if="isOrg">
      ...
      <section class="px-2 py-2 border-bottom-darken-10" v-if="orgSlug && hasDashboards">
        ...
        <nav class="nav nav-pills text-truncate flex-column">
          ...
          <router-link class="nav-link" :to="{name: 'orgSlug-dashboards-dashSlug', params: {orgSlug: orgSlug, dashSlug: dashboard.slug}}" :key="dashboard._id" v-for="dashboard in $store.state.dashboards">{{ dashboard.name }}</router-link>
        </nav>
      </section>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      isOrg: 'organization/hasCurrent',
      orgName: 'organization/name',
      orgSlug: 'organization/slug',
      orgTint: 'organization/tint'
    })
  },
